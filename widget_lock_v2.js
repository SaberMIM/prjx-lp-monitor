...
function formatUpdateTime(updatedAt) {
  if (!updatedAt) return "Maj --:--";
  const d = new Date(updatedAt);
  return `Maj ${d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
}
...
const updateLabel = formatUpdateTime(data.updatedAt);
...
if (config.widgetFamily === "accessoryInline") {
  ...
  main.addSpacer(2);
  addText(main, updateLabel, 10, false, 0.5);
}
else {
  const main = w.addStack();
  main.layoutVertically();
